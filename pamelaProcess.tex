...
\begin{itemize}
    ...
    \vspace{-0.2cm}\item \textbf{Reengineering} : programmers start from an existing base of code (legacy) and refactor it while replacing existing code by abstract method declarations in Java interface, reducing risks of errors.
    \vspace{-0.2cm}\item \textbf{Aspect-oriented programming} : programmers may use or redefine "patterns" (eg Security Patterns) which offers code weaving at runtime, as well as runtime monitoring.
    \vspace{-0.2cm}\item \textbf{Advanced programmers use} : programmers may extends PAMELA with their own annotations, their own implementations, their own patterns, etc...
\end{itemize}
}
